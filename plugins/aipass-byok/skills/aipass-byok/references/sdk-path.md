...
Do not block the generation action because `AiPass.isAuthenticated()` is false or wait for `aipass:login`; that prevents the SDK from opening its automatic login modal and resuming the operation. Catch `AUTH_REQUIRED` only when the user dismisses the modal.
...
After implementation, distinguish safe local proof from paid verification. A successful build or lint run means "implemented and built; live wallet-funded verification pending" until the user approves and performs a real call. Preserve the project's existing deployment target.
