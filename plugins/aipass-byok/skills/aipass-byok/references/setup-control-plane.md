...
Request only `setup:read`, `oauth-clients:read`, and `oauth-clients:create`. The OAuth callbacks remain exact. Request runtime `profile:read` only when ensuring a public client that intentionally uses AI Pass as host login.
...
Always send `setupVersion: 5` when following this version of the skill. It fails closed when `oauth-clients:create` lacks exact proposed callbacks. Omitting the field is reserved for compatibility with older published instructions.
...
```json
{
  ...
  "tokenType": "Bearer",
  "expiresIn": 2592000,
  "scopes": ["setup:read", "oauth-clients:read", "oauth-clients:create"]
}
```
...
Unwrap the standard response envelope's `data`. It is intentionally minimal: owned public OAuth clients with client type, runtime scopes, exact redirect URIs, and grant bounds. It contains no client secret, wallet balance, session token, or private profile.
...
## 5. Keep one grant across project setup work

Keep one grant in process memory through integration, correction, and retry for the same approved project. Do not revoke it merely after provisioning the OAuth client. Never persist it or use it outside the project resources shown on the approval page. Device approval does not authorize paid model calls.
